...
async function generateNpmPAT() {
    ...
    if (os.platform() === "win32") {
        ...
        const generatePAT = "vsts-npm-auth -F -R -E 43200 -C .npmrc";
...
async function checkNonWindowRequirements(todoList) {
    ...
    1. Ensure ".NET SDK" is installed and dotnet.exe is available on your path,
        you can download it from https://dotnet.microsoft.com/download
        If you need to Install .NET on Ubuntu,
        you can install by following: https://learn.microsoft.com/dotnet/core/install/linux-ubuntu`;
    const requirements = new Map([
...
async function oneTimeConfigurationSteps() {
    const commonRequirements = new Map([
        ...
        ["node", `node:: Please ensure "NodeJS" is available on the path, you can download the latest from https://nodejs.org/en/download/`],
        ["npm", `npm:: Please ensure "NodeJS" is available on the path, you can download the latest from https://nodejs.org/en/download/`],
    ]);
    ...
    7. From the command prompt in the same directory:
        ...
        - generate a readonly PAT using vsts-npm-auth (copy and paste below line to command prompt)
            vsts-npm-auth -F -R -E 43200 -C .npmrc
